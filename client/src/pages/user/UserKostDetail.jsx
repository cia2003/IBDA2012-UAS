...
function UserKostDetail() {
  ...
  const { getKostById, getStaffById } = useManagerContext();
  ...
  const fetchKostInfo = useCallback(async () => {
    try {
      ...
      if (result) {
        ...
        if (result.staffId) {
          const staffData = await getStaffById(result.staffId);
          if (staffData) {
  ...
  }, [getKostById, getStaffById, kostId]);
          ...
          <div className="bg-white p-6 rounded-3xl border border-zinc-100 shadow-sm sticky top-24">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col md:flex-row lg:flex-col gap-3 justify-center items-center w-full">
                <a
                  href={`https://wa.me/${employee.phone?.replace(/\D/g, "") || ""}`} // Membersihkan karakter non-angka
                  target="_blank"
                  rel="noreferrer"
                ...
                <a
                  href={`mailto:${employee.email || ""}?subject=Tanya Kost: ${kostData.name}`}
