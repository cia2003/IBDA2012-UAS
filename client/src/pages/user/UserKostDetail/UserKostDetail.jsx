...
import { useManagerContext, useUserContext, useStaffContext } from "../../../hook/useContext";
...
function UserKostDetail() {
  ...
  const { getUnauthenticatedKostDetail, getUnauthenticatedRooms, getUnauthenticatedRoomTypes } = useUserContext();
  ...
  const fetchKostInfo = useCallback(async () => {
    try {
      const data = await getUnauthenticatedKostDetail(kostId);
      const responsibleStaffData = await getStaffDataByKostId(kostId);

      const roomResponse = await getUnauthenticatedRooms();

      const roomTypes = await getUnauthenticatedRoomTypes();

      // Karena data hanya satu
      const kostWithRooms = {
        ...data,
        rooms: roomResponse
          .filter(room => room.kost === data.id)
          .map(room => {
            const roomType = roomTypes.find(type => type.id === room.room_type);
            return { ...room, room_type: roomType, phone: responsibleStaffData.phone };
          })
      };


      setKostData(kostWithRooms || []);
    } catch (error) {
  ...
  useEffect(() => {
    // const getRooms = kostData?.rooms && kostData?.rooms.length > 0 ? (
    //           kostData?.rooms.map((room) => (
    //             console.log(room.id),
    //             console.log(room.name), 
    //             console.log(room.room_type.size)
    // ))):


    // console.log("Ruang kamar: ", kostData?.rooms);
  });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-zinc-100 shadow-sm sticky top-24">
            <img
              src={kostData.thumbnail || kostData.image}
              alt={kostData.name}
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col md:flex-row lg:flex-col gap-3 justify-center items-center w-full">
                  <a
                    href={`https://wa.me/${kostData.phone?.replace(/\D/g, "") || "082134567789"}`} // Membersihkan karakter non-angka
                    target="_blank"
                    rel="noreferrer"
                ...
                <a
                  href={`mailto:${kostData.email || "customerservice@ibdakost.com"}?subject=Tanya Kost: ${kostData.name}`}
