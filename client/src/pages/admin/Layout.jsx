...
import { BedDouble, UsersRound, LayoutDashboard, DoorOpen, Building2, UserCheck } from "lucide-react";

const Layout = () => {
    const { staffData, logout } = useAppContext();

    const sidebarLinks =[
        { name: "Dashboard", path: `/dashboard/${staffData?.id}`, icon: <LayoutDashboard />
        },
        { name: "Kamar", path: `/dashboard/${staffData?.id}/kamar`, icon: <DoorOpen />},
        { name: "Penghuni", path: `/dashboard/${staffData?.id}/penghuni`, icon: <UsersRound />},
        { name: "Daftar Penghuni Baru", path: `/dashboard/${staffData?.id}/penghuni-baru`, icon: <UserCheck />}
    ]
